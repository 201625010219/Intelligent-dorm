...
extern DateStruct datedemo;
extern TimeStruct Alarm_Timedemo;
...
void RTC_TimeAndDate_Set(void)
{
    ...
    RTC_TimeStructure.Hours = (datedemo.hour[0]-0x30)*10+(datedemo.hour[1]-0x30);        
    RTC_TimeStructure.Minutes = (datedemo.minute[0]-0x30)*10+(datedemo.minute[1]-0x30);   
    RTC_TimeStructure.Seconds =(datedemo.second[0]-0x30)*10+(datedemo.second[1]-0x30);   
    RTC_TimeStructure.DayLightSaving = RTC_DAYLIGHTSAVING_NONE;
    RTC_TimeStructure.StoreOperation = RTC_STOREOPERATION_RESET;
    HAL_RTC_SetTime(&Rtc_Handle,&RTC_TimeStructure, RTC_FORMAT_BIN);
    // 初始化日期	
    //RTC_DateStructure.WeekDay = WEEKDAY;       
    RTC_DateStructure.Date = (datedemo.day[0]-0x30)*10+(datedemo.day[1]-0x30);         
    RTC_DateStructure.Month = (datedemo.month[0]-0x30)*10+(datedemo.month[1]-0x30);         
    RTC_DateStructure.Year =  (datedemo.year[2]-0x30)*10+(datedemo.year[3]-0x30);            
...
void RTC_TimeAndDate_Show(void)
{
    ...
    {
			// 打印日期
			printf("The Date :  Y:20%0.2d - M:%0.2d - D:%0.2d \r\n", 
...
void RTC_AlarmSet(void)
{
	...
	RTC_AlarmStructure.AlarmTime.Hours   = (Alarm_Timedemo.hour[0]-0x30)*10+(Alarm_Timedemo.hour[1]-0x30);
	RTC_AlarmStructure.AlarmTime.Minutes = (Alarm_Timedemo.minute[0]-0x30)*10+(Alarm_Timedemo.minute[1]-0x30);
	RTC_AlarmStructure.AlarmTime.Seconds = (Alarm_Timedemo.second[0]-0x30)*10+(Alarm_Timedemo.second[1]-0x30);
	RTC_AlarmStructure.AlarmMask =  RTC_ALARMMASK_DATEWEEKDAY;
	RTC_AlarmStructure.AlarmDateWeekDaySel = ALARM_DATE_WEEKDAY;
